...
// 		G4double pDphi); // segment angl
	...
	if(Use_DSL==1)//DSL1 is used
	{
		...
		logicDSSD1_Frame
			= new G4LogicalVolume(solidDSSD1_Frame,copper,"logicDSSD1_Frame");
		physiDSSD1_Frame
		...
		logicDSSD2_Frame
			= new G4LogicalVolume(solidDSSD2_Frame,copper,"logicDSSD2_Frame");
		physiDSSD2_Frame
		...
		G4VSolid* solidPlate
			= new G4Box("solidPlate",44.45/2.*mm,64.5/2.*mm,1.6/2.*mm);
		G4VSolid* solidAperture
			= new G4Tubs("solidAperture",0.*mm, 3.2*mm, 1.61/2.*mm,	0.*deg, 	360.*deg); // slightly thicker
		G4SubtractionSolid* solidCollimator
			= new G4SubtractionSolid("solidCollimator",solidPlate, solidAperture, 0, G4ThreeVector(0.,0.,0.));
		logicCollimator
			= new G4LogicalVolume(solidCollimator,iron,"logicCollimator"); // A36 Mild/Low Carbon Steel  Composition: 98% Iron
		physiCollimator
			= new G4PVPlacement(0,G4ThreeVector(0,0,15*mm),logicCollimator,"physiCollimator",logicWorld,false,0,checkOverlaps);

		//DSL1 Si Detector mount
		G4VSolid* solidBlock
			= new G4Box("solidBlock",44.45/2.*mm,42.926/2.*mm,0.817*25.4/2.*mm);
		
		G4VSolid* solidApertureforD1
			= new G4Tubs("solidApertureforD1",0.*mm, (1.256*25.4/2.)*mm, (0.386*25.4)/2.+0.01*mm,	0.*deg, 	360.*deg); // slightly thicker
		G4SubtractionSolid* solidBlockwithApertureD1
			= new G4SubtractionSolid("solidBlockwithApertureD1",solidBlock, solidApertureforD1, 0, G4ThreeVector(0.,10.3124,-(0.817/2.-0.386/2.)*25.4*mm));
		
		G4VSolid* solidApertureforD2
			= new G4Tubs("solidApertureforD2",0.*mm, (1.028*25.4/2.)*mm, (0.336*25.4)/2.+0.01*mm,	0.*deg, 	360.*deg); // slightly thicker
		G4SubtractionSolid* solidBlockwithApertureD1D2
			= new G4SubtractionSolid("solidBlockwithApertureD1D2",solidBlockwithApertureD1, solidApertureforD2, 0, G4ThreeVector(0.,10.3124,(0.817/2.-0.336/2.)*25.4*mm));
		
		G4VSolid* solidApertureforBetween
			= new G4Tubs("solidApertureforBetween",0.*mm, (0.75*25.4/2.)*mm, (0.817-0.386-0.336)*25.4/2.+5*mm,	0.*deg, 	360.*deg); // slightly thicker
		G4SubtractionSolid* solidBlockwithApertureD1D2Between
			= new G4SubtractionSolid("solidBlockwithApertureD1D2Between",solidBlockwithApertureD1D2, solidApertureforBetween, 0, G4ThreeVector(0.,10.3124,0.));

		logicDetectorMount
			= new G4LogicalVolume(solidBlockwithApertureD1D2Between,teflon,"logicDetectorMount"); // A36 Mild/Low Carbon Steel  Composition: 98% Iron
		physiDetectorMount
...
// 			= new G4SubtractionSolid("solidBlockwithApertureD1",solidBlock, solidApertureforD1, 0, G4ThreeVector(0.,10.3124,-(0.817/2.-0.386/2.)*25.4*mm));
	...
	// 	G4VSolid* solidCollimator
	// 		= new G4Box("solidCollimator",50./2.*mm,50./2.*mm,300./2.*um);
	logicChamber
		= new G4LogicalVolume(solidChamber,aluminum,"logicChamber"); //6061 Aluminum Alloy Composition: 98% Aluminum
	physiChamber
		= new G4PVPlacement(Rot,G4ThreeVector(0,0,0*mm),logicChamber,"physiChamber",logicWorld,false,0,checkOverlaps);
