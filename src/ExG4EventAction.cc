...
//   }
  ...
  if(dHC1)//For DSSD1
  {
	  ...
	  {
		  ih=-100;	jh=-100;	if(i>0) break;//using this statement for position distribution, the energy won't be correct!
		  ExG4Hit* hit = (*dHC1)[i];//fHitsCollection中第i个hit的信息，有时一个event就产生1个hit，但打出次级粒子会有多个hit
		  //hit->Print();
		  tof = hit->GetTof();
		  G4ThreeVector globalPos=hit->GetPos();
		  ih = static_cast<int>((globalPos.x()+24.8)/3.1);
		  jh = static_cast<int>((globalPos.y()+24.8)/3.1);
  ...
  if(dHC4)//For QSD1
  {
	  ...
	  for(G4int i=0;i<n_hits;i++)//Within this loop, the histo are filled by hit, so the number can be much larger than the event number, the nt can only be filled by event (the last hit)
	  {
		  ...
		  tof = hit->GetTof();
		  G4ThreeVector globalPos=hit->GetPos();
		  ih = static_cast<int>((globalPos.x()+23.3)/3.1);
		  jh = static_cast<int>((globalPos.y()+23.3)/3.1);
