...
// 	G4cout<<"brtot=	"<<branch_lit_tot<<G4endl;
	...
	depth_3He = new TH1F("depth_3He","depth_3He", 100,1.5,301.5);//unit nm, this statement cannot be eliminated, the new TH1F cannot be named the same as the TH1F in the input root
...
//	xygaus->GetRandom2(x0,y0); // 2D Gaussian beam spot  // Return two random numbers to x0 and y0 following this function shape.
	...
	z0=depth_3He->GetRandom()*CLHEP::nm;//Return a random number distributed according the histogram
...
// 	Excitation_recoil=4144*CLHEP::keV; tau=30; //fs // 30P
	...
	tau=1.442695*tau/1000*CLHEP::picosecond; // don't change
//	cout<<tau/CLHEP::ns<<G4endl;
	ioncharge = 0.*CLHEP::eplus; // don't change

//	Excitation_recoil=3335*CLHEP::keV; // an annoying mean-max correction in EMG function; optional
	...
	//	tau=150; // fs
	//	tau=1.442695*tau/1000000*CLHEP::ns; // don't change
	particle = G4IonTable::GetIonTable()->GetIon(Zrecoil,Arecoil,Excitation_recoil);
